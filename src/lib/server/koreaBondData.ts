...
async function fetchByParam(
  serviceKey: string,
  paramName: "bondIsurNm" | "scrsItmsKcdNm",
  keyword: string
 ...
 * 국내 채권을 검색한다. 발행회사명(bondIsurNm) 부분일치와 종목종류
 * (scrsItmsKcdNm, 예: 국채/지방채/특수채/금융채/일반회사채) 부분일치를 함께
 * 조회해 합친다 — "국채"처럼 특정 회사명이 아닌 채권 구분으로 찾는 경우까지
 * 포괄한다(국공채는 발행인명이 "대한민국"으로 등록되어 있어 이름만으로는
 * 검색되지 않는다). 서비스키는 DATA_GO_KR_BOND_SERVICE_KEY 환경변수로 관리한다.
...
export async function searchKoreaBonds(keyword: string): Promise<KoreaBondItem[]> {
  ...
  const [byIssuer, byCategory] = await Promise.all([
  ...
  for (const item of [...byIssuer, ...byCategory]) {
